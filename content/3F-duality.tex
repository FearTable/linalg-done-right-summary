...
%\textbf{3.112}
\begin{mydef}
  If $\onetillm{v}$ is a basis of $V$, then the \qt{dual basis} of $\onetilln{v}$ is the list $\varphi_1, \dots, \varphi_m \in V^{*}$, where each $\varphi_j$ is the linear functional such that
  \begin{equation}
...
\setcounter{thm}{113}
\begin{thm}
  Let $\onetillm{v}$ be a basis of $V$. $\onetillm{\varphi}$ is called \qt{a dual basis} of $\onetillm{v}$ because for every $v \in V$ we have
  \begin{equation}
...
%\textbf{3.118}
\setcounter{thm}{117}
\begin{mydef}[dual map, $T'$]
  The \qt{dual map} $\dual{T}$ of $T \in \linmap(V,W)$ is the linear map $T' \in \lin{W'}{V'}$ defined like this:
\begin{equation}
