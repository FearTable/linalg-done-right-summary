...
\begin{mydef}[dual map, $\dual{T}$]
  The \qt{dual map} $\dual{T}$ of $T \in \linmap(V,W)$ is the linear map $\dual{T} \in \lin{W'}{V'}$ defined like this:
\begin{equation}
    \dual{T}(\varphi) :\equiv \varphi \circ T \quad \forall \phi \in \dual{W}.
\end{equation}

    So we have for a $\varphi \in \dual{W}=\lin{W}{\mathbb{F}}$ that $T'(\varphi) \in V' = \lin{V}{\mathbb{F}}$. So $\dual{T} \in \linmap(V,W)$ is indeed a map from $W'$ to $V'$
