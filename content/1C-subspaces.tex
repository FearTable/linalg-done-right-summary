...
\begin{proof}
  \begin{description}
    ...
    \end{equation}
    $\implies v=0$ becauuse $U+W$  is a direct sum. Therefore $U \cup W = \{0\}$
    \item{$\Leftarrow$ direction:} Suppose $U\cup W = \{0\}$. Let  $u\in U$ and $w \in W$ such that
    \begin{equation}
      0 = u+w
    \end{equation}
    By \ref{condition-for-a-direct-sum} we know that our gould is to show that $u=w=0$. The equation above implies that $u=-w\in W$  and $w=-u \in U$ for all arbitrary choices of $U$ and $W$. Thus $u,w \in U\cup W \implies u=w=0.$ Therefore, $U+W$ is a direct sum.
  \end{description}
