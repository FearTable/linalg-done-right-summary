...
\begin{prf}
  ...
  \begin{equation}
    u+w = (v_1 + \cdots + v_m) + (v'_1 + \cdots + v'_m) =  \underbrace{(v_1+v'_1)}_{\in V_1} + \cdots + \underbrace{(v_m+v'_m)}_{\in V_m} \in  V_1 + \cdots + V_m \in V.
  \end{equation}
...
\begin{prf}
    ...
    \qt{$\Leftarrow$ direction:} Now suppose that the only way to write $0$ as a sum $w_1 + \cdots + w_m$ where each $w_k \in V_k$, is by taking each $w_k = 0$. Let $v \in V$ \st $v$ has two representations denoted as
    \begin{equation}
      \begin{aligned}
