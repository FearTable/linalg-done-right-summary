...
\end{thm}
\begin{prf}
  First we need to show, that $V_1 + \cdots + V_m$ is a subspace of $V$. Clearly, the additive identity $0 = 0 + \cdots + 0$ belongs to $V_1 + \cdots + V_m$.
  Next we need to show that our sum is closed under addition. Let $u,w \in V_1 + \cdots + V_m$.  Thus, we have $u = v_1 + \cdots + v_m$ \st $v_1 \in V_1, \ddd, v_m \in V_m$ and $w=v'_1 + \cdots + v'_m$ \st $v'_1, \ddd , v'_m \in V_m$. Therefore,
  \begin{equation}
    u+w = (v_1 + \cdots + v_m) + (v'_1 + \cdots + v'_m) =  \underbrace{(v_1+v'_1)}_{\in \, V_1} + \cdots + \underbrace{(v_m+v'_m)}_{\in V_m} \in \, V_1 + \cdots + V_m.
  \end{equation}

  Hence, $V_1 + \cdots + V_m$ is closed under addition.
  For $\lambda \in \myF$ we have that
  \begin{equation}
      \lambda u = \lambda(v_1 + \cdots + v_m) = \underbrace{\lambda v_1}_{\in \, V_1} + \cdots + \underbrace{\lambda v_m}_{\in \, V_m} \in V_1 + \cdots + V_m,
  \end{equation}

  so it is closed under scalar multiplication. Therefore, $V_1 + \cdots + V_m$ fullfills all critieria for a subspace accoring to \ref{thm: conditions for a subspace}.
  Now we have to show the minimality of $V_1+\cdots+V_m$ and that $V_1+\cdots+V_m$ is containded in every subspace of $V$.
  The subspaces $V_1, \ddd, V_m$ are all contained in $V_1+\cdots+V_m$, because $\forall \kInOneTillM:$
  \begin{equation}
    V_k = \{ v \mid v \in V_k \} \subseteq \{v_1 + \cdots + v_m \mid v_1 \in V_1,  \ddd , v_m \in V_m \} = V_1 + \cdots + V_m.
  \end{equation}

  Conversely, every subspace of $V$ containing $V_1, \ldots, V_m$ contains $V_1 + \cdots + V_m$, because $V$ is closed under addition. Thus, $V_1 + \cdots + V_m$ is the smallest subspace of $V$ containing $V_1, \ldots, V_m$.
\end{prf}
...
\begin{prf}
    ...
    \qt{\Rightarrowdirection} First suppose $V_1 + \cdots + V_m$ is a direct sum. Let $0 = v_1 + \cdots + v_m,$ where each $v_k \in V_k$.
    ...
    \Leftarrowdirection Now suppose that the only way to write $0$ as a sum $w_1 + \cdots + w_m$ where each $w_k \in V_k$, is by taking each $w_k = 0$. Let $v \in V$ \st $v$ has two representations denoted as
    \begin{equation}
      \begin{aligned}
        v& :\equiv v_1 + \cdots + v_m \where v_1 \in V_1, \ddd, v_m \in V_m \myand \\
        v& :\equiv u_1 + \cdots + u_m  \where u_1 \in V_1, \ddd, u_m \in V_m
      \end{aligned}
    \end{equation}
    ...
    Because $v_1 - u_1 \in V_1, \ldots, v_m - u_m \in V_m$, the equation above implies that $\forall k \in \{1, \ddd, m\}$: $v_k -u_k$ equals $0$.
    \begin{equation}
      \implies v_1=u_1, \ddd, v_m=u_m
    \end{equation}

    So there is only one way to write $v$ as a sum. So $V_1 + \cdots + V_m$ is a direct sum.
\end{prf}
